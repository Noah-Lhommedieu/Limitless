...
public class JobAction
{
  ...
  public void startAction(Player P, InteractiveObject IO, GameAction GA, GameCase cell, JobStat SM)
  {
    ...
    if(P.getObjetByPos(Constant.ITEM_POS_ARME)!=null&&SM.getTemplate().getId()==36)
    {
      if(Main.world.getMetier(36).isValidTool(P.getObjetByPos(Constant.ITEM_POS_ARME).getTemplate().getId()))
      {
        ...
        if(distItem<dist)
        {
          SocketManager.GAME_SEND_MESSAGE(P,"Vous �tes trop loin pour attraper du poisson!");
        ...
        if(_tID!=-1)
        {
          ...
          if(_T!=null)
          {
            ...
            if(SM.getTemplate().getId()==JobConstant.JOB_PECHEUR)
              SocketManager.GAME_SEND_MESSAGE(player,"Vous venez de p�cher un poisson rare.","009900");
            else if(SM.getTemplate().getId()==JobConstant.JOB_MINEUR)
              SocketManager.GAME_SEND_MESSAGE(player,"Lors de l'exploitation mini�re, vous remarquez quelque chose qui scintille dans la roche.","009900");
            else if(SM.getTemplate().getId()==JobConstant.JOB_PAYSAN)
              SocketManager.GAME_SEND_MESSAGE(player,"Vous venez de r�colter une c�r�ale rare.","009900");
          }
        }
      }
  ...
  private int addCraftObject(Player player, GameObject newObj)
  {
    for(Entry<Integer, GameObject> entry : player.getItems().entrySet())
    {
      ...
      {
        obj.setQuantity(obj.getQuantity()+newObj.getQuantity(),player); //On ajoute QUA item a la quantit� de l'objet existant
...
public boolean doMage(boolean repeat, Player receiver, Map<Player, ArrayList<Pair<Integer, Integer>>> items)
  {
   ...
   // System.out.println("exoPower: "+exoPower);
    ...
    if(rune!=null)
    {
      if(Job.getActualJet(objectFm,rune.getStatId())+rune.getStatsAdd()>getStatBaseMaxs(objectFm.getTemplate(),rune.getStatId())) //current mage exceeds base stats
        ...
        {
         // SocketManager.GAME_SEND_MESSAGE(this.player,"Ce mage d�passerait la limite de puissance maximale autoris�e.");
          if(receiver!=null)
          {
        	  canFM=false;
          }
    ...
    else
    {
      ...
      //v2.8 - smarter max stat calculation
       ...
       if(statMax == 0) {
    	  ...
    	  {
    		  //SocketManager.GAME_SEND_MESSAGE(this.player,"Ce mage d�passerait la limite de statistiques maximale autoris�e");
    		  canFM = false; 	  
    	  }
       }else
      if(currentStat+rune.getStatsAdd()>maxOvermage)
      {
        SocketManager.GAME_SEND_MESSAGE(this.player,"Ce fm d�passerait la limite de statistiques maximale autoris�e "+maxOvermage+".");
   ...
   // }
    ...
    if(successC)
    {
      ...
      if(!secure)
        SocketManager.GAME_SEND_MESSAGE(this.player,"Vous avez fm avec succ�s.","009900");
      SocketManager.GAME_SEND_Ec_PACKET(this.player,"EF");
    ...
    if(!successC)
    {
      if(lostPower.getLeft()!=-1)
      {
        ...
        {
          if(successN)
            SocketManager.GAME_SEND_MESSAGE(this.player,"Vous avez r�ussi � fm cet objet - Le puits actuel de cet objet est "+newSink+".","009900");
          else
            SocketManager.GAME_SEND_MESSAGE(this.player,"Vous n'avez pas r�ussi � fm cet objet - Le puits actuel de cet objet est "+newSink+".","009900");
          objectFm.setPuit(newSink);
        }
        else if(successN)
          SocketManager.GAME_SEND_MESSAGE(this.player,"Vous avez r�ussi � fm cet objet, mais vous avez perdu certaines statistiques.","009900");
        else
          SocketManager.GAME_SEND_MESSAGE(this.player,"Vous n'avez pas r�ussi � fm cet �l�ment.","009900");
      }
      else if(successN)
        SocketManager.GAME_SEND_MESSAGE(this.player,"Vous avez r�ussi � fm cet objet, mais vous avez perdu certaines statistiques.","009900");
      else
