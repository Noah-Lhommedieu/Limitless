...
public class CommandAdmin extends AdminUser
{
  ...
  public void command(String command, String[] infos, String msg)
  {
    ...
    else if(command.equalsIgnoreCase("DELJOB"))
    {
      ...
      Database.getStatics().getPlayerData().update(perso);
      SocketManager.GAME_SEND_MESSAGE(perso,"Vous venez d'apprendre un mÈtier, veuillez vous reconnecter pour finaliser les changements.");
    ...
    else if(command.equalsIgnoreCase("LEVEL"))
    {
      ...
      Constant.onLevelUpSpells(perso, perso.getLevel()); // le possËde Player.SpellMax(perso);
      perso.ItemEvolution();
    ...
    else if(command.equalsIgnoreCase("ADDOMGEXP")) // Ajoute au joueur une quantitÈ d'exp omega
    {
      try
      {
    ...
    else if(command.equalsIgnoreCase("SETOMGLVL")) // Met le joueur au lvl omega donnÈ
    {
      try
      {
    ...
    else if(command.equalsIgnoreCase("SHUTDOWN"))
    {
      ...
      else if(OffOn==1&&!this.isTimerStart())
      {
        if(time<=15)
        {
          for(Player player : Main.world.getOnlinePlayers())
          {
        	  SocketManager.PACKET_POPUP_DEPART(player, "Vous ne pouvez pas lancer de combats car le serveur redÈmarrera bientÙt.");
            //player.sendServerMessage("Vous ne pouvez pas lancer de combats car le serveur redÈmarrera bientÙt.");
            //player.send("M13");
          }
      ...
      else if(OffOn==0&&this.isTimerStart())
      {
        ...
        for(Player player : Main.world.getOnlinePlayers())
          player.sendServerMessage("Le redÈmarrage a ÈtÈ annulÈ. Vous pouvez recommencer les combats.");
        Config.getInstance().fightAsBlocked=false;
    ...
    else if(command.equalsIgnoreCase("ADDORNA"))
    {
    	Player perso=this.getPlayer();
        if(infos.length==3)//Si le nom du perso est specifie
        {
          String name=infos[2]; // On rÈcupËre le joueur ciblÈ
          perso=Main.world.getPlayerByName(name); //RÈcupËre le Player si le joueur existe via son nom
          if(perso==null)
            perso=this.getPlayer(); // Si le joueur ciblÈ existe pas, on prend le joueur qui execute la commande
        }
    	String stringorn=infos[1]; // On rÈcupËre l'ornement 
        ...
        if(perso==null) // Si la cible choisi existe pas
          perso=this.getPlayer(); // On prend le joueur qui ‡ fait la commande
        perso.addOrnements(ornement); // Ajoute l'ornement en BDD au joueur
        perso.setOrnement(ornement); // Equipe l'ornement donnÈ, car c'est stylÈ
    	perso.sendMessage(perso.getName() + ": Ornement reÁu -> " + perso.getOrnement()); // Affichage rÈsultat d'obtention d'ornement
    	SocketManager.GAME_SEND_ALTER_GM_PACKET(perso.getCurMap(), perso);
        perso.refresh();
