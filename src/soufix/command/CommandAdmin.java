...
public class CommandAdmin extends AdminUser
{
  ...
  public void command(String command, String[] infos, String msg)
  {
    ...
    else if(command.equalsIgnoreCase("ADDOMGEXP")) // Ajoute au joueur une quantit� d'exp omega
    {
      try
      {
    ...
    else if(command.equalsIgnoreCase("SETOMGLVL")) // Met le joueur au lvl omega donn�
    {
      try
      {
    ...
    else if(command.equalsIgnoreCase("ADDORNA"))
    {
        ...
        if(infos.length==3)//Si le nom du perso est specifie
        {
          String name=infos[2]; // On r�cup�re le joueur cibl�
          perso=Main.world.getPlayerByName(name); //R�cup�re le Player si le joueur existe via son nom
          if(perso==null)
            perso=this.getPlayer(); // Si le joueur cibl� existe pas, on prend le joueur qui execute la commande
        }
    	String stringorn=infos[1]; // On r�cup�re l'ornement 
        ...
        perso.setOrnement(ornement); // Equipe l'ornement donn�, car c'est styl�
